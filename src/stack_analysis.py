...
class StackAnalysis:
    ...
    def analyze(
        ...
        for _, foil in self.stack.stack.iterrows():
            ...
            if target not in products:
                if not silent:
                    print(
                        f"Element {target} not found in products: {products.keys()}. Skipping."
                    )
            ...
            for _, isotope in products[target].iterrows():
                ...
                for _, gamma in iso.gammas(
                    ...
                    if np.isnan(t_c).any():
                        if not silent:
                            print(
                                f"Skipping NaN value for isotope {iso_name} from target {target} with energy {E_γ} MeV."
                            )
                        continue
    ...
    def plot(
        ...
        plot_row = lambda row: plt.plot(
            row["t_d"],
            row["t_c"],
            label=rf"E: {row['gamma_energy']} keV, iso: $^{{{row['isotope'][:-2]}}}${row['isotope'][-2:]}, target: {row['target']}, foil: {row['foil_number']}",
        )
    ...
    def _A0(
        ...
        """
        if (
            isinstance(t_irr, np.ndarray)
            and isinstance(λ, np.ndarray)
            and t_irr.shape != λ.shape
        ):
